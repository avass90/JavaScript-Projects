...
//Diable the stop button since it is not needed until game start.
...
function rollForTurn() {
    ...
    // determined and concatenate string showing which player won the roll
    if (pOne > pTwo) {
        first = "Player 1";
...
// reset all squares to their startign empty state.
...
//This function wil conceal the message console from view
...
function squareSound() {
    ...
    sound.play();
    setTimeout(function() {sound.pause();},400); //add delay to keep the sounds short
...
function checkWinCon1(info,squareArray) {
    var winDetected = "on";
    var winCon1 = [0,1,2];
    //iterate throughh the growing array during
    //gametime searching for the existence of
    //inex 0, index 1, and index 2 and once they
...
function checkWinCon2(info,squareArray) {
    ...
    for (var i in info) {
        if (info[i].charAt(0) == "3") {
            var match3Avatar = info[i].charAt(1); //only interest in recording the avatar
        }
...
//checking for wincon square 147
...
function animateO(selected) {
    selected.style.transform = (selected.style.transfrom == "translateY(0%)" || null) ? "translateY(0%)" : "translateY(0%)";
}

//this function will performt he animation for the X avatar.
